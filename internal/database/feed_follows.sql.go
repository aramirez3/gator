...
const deleteFeedFollowsForUserAndFeed = `-- name: DeleteFeedFollowsForUserAndFeed :exec
DELETE FROM feed_follows
WHERE 
    user_id = $1 AND
    feed_id = $2
...
type DeleteFeedFollowsForUserAndFeedParams struct {
	UserID uuid.UUID
	FeedID uuid.UUID
}

func (q *Queries) DeleteFeedFollowsForUserAndFeed(ctx context.Context, arg DeleteFeedFollowsForUserAndFeedParams) error {
	_, err := q.db.ExecContext(ctx, deleteFeedFollowsForUserAndFeed, arg.UserID, arg.FeedID)
	return err
}
